...
import { CircleCheckBig, MailCheckIcon } from "lucide-react";
...
export default function ForgotPasswordAdmin() {
  ...
  const [apiMessage, setApiMessage] = useState(""); // Combined message state
  ...
  const onSubmit = async (values: ForgotPasswordFormData) => {
    ...
    try {
      const response = await fetch("/api/adminData/forgot-password", {
        ...
        body: JSON.stringify({ email: values.email.toLowerCase().trim() }), // Use validated email
      });
      ...
      <Card className="w-full max-w-md shadow-lg">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <div className="relative">
                          <MailCheckIcon className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                          <Input
                            type="email"
        ...
        <CardFooter className="flex flex-col items-center justify-center pt-6 border-t">
          <Button variant="link" asChild className="text-sm font-medium">
            <Link href="/adminData/LoginAdmin">Back to Login</Link>
          </Button>
        </CardFooter>
      </Card>
    </div>

...
export default function ManageTasksPage() {
  ...
  const [taskModalData, setTaskModalData] = useState<Task | null>(null); // Renamed state
  ...
  const handleMarkPending = async () => {
    ...
    setShowMarkPendingConfirm(true); // Show confirmation dialog
  };
  ...
  const confirmMarkPending = async () => {
    ...
    try {
      ...
      setTasks((prevTasks) =>
        prevTasks.map((task) =>
          task.TaskId === taskModalData.TaskId
            ? { ...task, status: "Re Assigned", context: feedback } // Update locally
            : task
        )
      );
      ...
      setShowMarkPendingConfirm(false); // Close confirmation dialog
  ...
  const getCardBgColor = (status: string) => {
    // Use ring for selection, background for status
    switch (status) {
      case "Pending":
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "Pending":
        return <Clock className="h-3 w-3" />; // Smaller icon
      case "In Progress":
  ...
  // Task Click Handler (Opens Modal or Selects)
  const handleTaskClick = (task: Task) => {
    if (isSelectMode) {
      toggleTaskSelection(task.TaskId);
    } else {
      setTaskModalData(task); // Open the details modal
    }
  };

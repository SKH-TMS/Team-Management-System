...
export default function UserProfile() {
  ...
  const handleUpload = async () => {
    ...
    formDataToUpload.append("email", currentUserData.email);
    try {
      const res = await fetch("/api/upload/update-profile-pic", {
        method: "POST",
  ...
  const handleProfileUpdateSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/userData/profile/update-info", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      if (data.success && data.user) {
        ...
        setCurrentUserData((prev) => (prev ? { ...prev, ...data.user } : null));
      } else {
  ...
  const handlePasswordChangeSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/userData/profile/change-password", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div className="w-full space-y-5">
      {/* User Profile Header & Picture Upload */}
      <Card className="overflow-hidden shadow-md">
        <CardHeader className="bg-gradient-to-r from-slate-50 to-slate-100 p-4 border-b">
          <div className="flex flex-col items-center sm:items-start md:flex-row gap-4">
            <div className="relative group">
              <Avatar className="h-24 w-24 border-4 border-white shadow-md">
                <AvatarImage
                  src={previewUrl || currentUserData.profilepic}
            ...
            <div className="text-center sm:text-left flex-1">
              <div className="flex flex-col sm:flex-row sm:items-center gap-1">
                ...
                <Badge
                  variant="secondary"
                  className="self-center my-1 sm:my-0 sm:ml-2"
                >
                  <Shield className="w-3 h-3 mr-1" /> {currentUserData.userType}
                </Badge>
              </div>
              ...
              <CardDescription className="mt-2">
                ...
                {currentUserData.contact && (
                  <div className="flex items-center justify-center sm:justify-start gap-2 mt-1 text-sm text-slate-600">
                    <Phone className="w-4 h-4 flex-shrink-0" />
                    <span>{currentUserData.contact}</span>
                  </div>
                )}
              </CardDescription>
            </div>
          ...
          {selectedFile && (
            <div className="mt-4 flex flex-col items-center sm:items-start md:flex-row md:justify-end gap-3 bg-slate-50 p-3 rounded-lg border">
      ...
      <Card className="shadow-md overflow-hidden">
        <Tabs defaultValue="personal" className="w-full">
          <TabsList className="w-full rounded-none border-b grid grid-cols-2">
            <TabsTrigger
              value="personal"
              className="rounded-none data-[state=active]:border-b-2 data-[state=active]:border-primary py-2 text-xs sm:text-sm"
            >
          ...
          <TabsContent value="personal" className="p-0 m-0">
            <CardContent className="pt-4 sm:pt-6 px-3 sm:px-6">
              ...
              <form onSubmit={handleProfileUpdateSubmit} className="space-y-4">
                ...
                <div className="space-y-2 pt-4 border-t">
                  <div className="flex flex-col sm:flex-row sm:items-center text-sm text-slate-600 gap-1">
                    <div className="flex items-center">
                      <Mail className="w-4 h-4 mr-2 text-slate-400" />
                      <span className="font-medium mr-2">Email:</span>
                    </div>
                    <span className="break-all pl-6 sm:pl-0">
                      {currentUserData.email}
                    </span>
                    <Badge
                      variant="outline"
                      className="ml-0 mt-1 sm:mt-0 sm:ml-2 text-xs w-fit"
                    >
                      Cannot change
                    </Badge>
                  </div>
            ...
            <CardFooter className="flex flex-col sm:flex-row sm:justify-between pt-2 border-t bg-slate-50 px-3 sm:px-6 py-3 gap-2">
      ...
      <Card className="shadow-md">
        <CardHeader className="pb-2 sm:pb-3 px-4 sm:px-6 pt-4 sm:pt-5">
          <div className="flex items-center">
            <Shield className="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-slate-500" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm sm:text-base font-medium mt-1 flex items-center">
              ...
              <div>
                ...
                <p className="text-sm sm:text-base font-medium mt-1 flex items-start">
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm sm:text-base font-medium mt-1 flex items-start">
                  ...
                  <span className="break-all">{currentUserData.UserId}</span>
                </p>
              </div>
